...
总入口只根据用户意图选择一个具体 Skill；它不执行 Runtime 预检、不启动 service、不打开 Studio，也不读写项目。两个业务 Skill 共用 `scripts/ensure-runtime.cjs`、`scripts/ensure-running.cjs` 和 `runtime-requirements.json`。Plugin package `0.3.1` 消费 Runtime compatibility contract `0.2.0`：只接受 Runtime 0.2.0+ 与声明的 EDL、常驻 service 能力；缺失时从精确的 `v0.2.0` Release 获取安装器和校验清单，校验后安装并执行 doctor。随后由 Product `service ensure --json` 幂等安装或恢复 macOS 用户服务；Plugin 不直接使用 `launchctl`、`nohup` 或 foreground 后台进程。Release 尚未发布、资产不完整或服务身份不匹配时安全停止，不会安装或覆盖旧 Runtime。Studio 只在人工审核状态且通过 `ensure-studio.cjs` 顶层视图能力门禁后打开。
...
Plugin 是独立的 `0.3.1` 候选版本；`runtime-requirements.json` 仍要求 Product Runtime `0.2.0`。两者不能互相替代。
