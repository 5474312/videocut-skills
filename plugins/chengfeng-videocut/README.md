...
公开入口由一个只路由的总入口、两个业务入口和两个支持入口组成：

```text
chengfeng-videocut -> 选择一个具体入口，不读写 Product
...
总入口只根据用户意图选择一个具体 Skill；它不执行 Runtime 预检、不启动 service、不打开 Studio，也不读写项目。顶层 Plugin 只保留运行时支持的三个 starter prompts；Bug 与更新仍通过各自可手动选择的 Skill 进入。两个业务 Skill 共用 `scripts/ensure-runtime.cjs`、`scripts/ensure-running.cjs` 和 `runtime-requirements.json`。Plugin package `0.3.2` 消费 Runtime compatibility contract `0.2.0`：只接受 Runtime 0.2.0+ 与声明的 EDL、常驻 service 能力；缺失时从精确的 `v0.2.0` Release 获取安装器和校验清单，校验后安装并执行 doctor。随后由 Product `service ensure --json` 幂等安装或恢复 macOS 用户服务；Plugin 不直接使用 `launchctl`、`nohup` 或 foreground 后台进程。Release 尚未发布、资产不完整或服务身份不匹配时安全停止，不会安装或覆盖旧 Runtime。Studio 只在人工审核状态且通过 `ensure-studio.cjs` 顶层视图能力门禁后打开。
...
Plugin 是独立的 `0.3.2` 候选版本；`runtime-requirements.json` 仍要求 Product Runtime `0.2.0`。两者不能互相替代。

## 手动入口

每个公开入口在 `SKILL.md` 标记 `user-invocable: true`。技能选择器应展示 `agents/openai.yaml` 中的可读名称；精确技术 ID 始终保留 Plugin namespace：
...
不要使用裸 `$chengfeng` 或复制无 namespace alias；它们不是本 Plugin 的公开 Skill ID。
