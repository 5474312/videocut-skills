...
const manifest = JSON.stringify({ name: "chengfeng-videocut", version: "0.3.1" }); fs.writeFileSync(path.join(bundle, ".codex-plugin", "plugin.json"), manifest); fs.writeFileSync(path.join(bundle, "payload.txt"), "candidate");
...
const candidate={name:'chengfeng-videocut',version:'0.3.1',bundlePath:bundle,installed:false,...(process.env.FAKE_UNTRUSTED?{publisherChecksum:'b'.repeat(64)}:{}),...(process.env.FAKE_REF?{immutableRef:process.env.FAKE_REF}:{})};
const installed=()=>({pluginId:'chengfeng-videocut@test',name:fs.existsSync(state)&&process.env.FAKE_AFTER_NAME?process.env.FAKE_AFTER_NAME:'chengfeng-videocut',version:fs.existsSync(state)?'0.3.1':'0.2.0',installed:true,...(fs.existsSync(state)&&process.env.FAKE_AFTER_REF?{immutableRef:process.env.FAKE_AFTER_REF}:{}),...(fs.existsSync(state)&&process.env.FAKE_AFTER_HASH?{publisherChecksum:process.env.FAKE_AFTER_HASH}:{})});
...
const activate = (extra = {}) => invoke(["--activate", "--confirmed", "--expected-version", "0.3.1", "--expected-ref", ref, "--expected-sha256", bundleHash], extra);
try {
  ...
  r = invoke(["--activate", "--confirmed", "--expected-version", "0.3.1", "--expected-ref", "wrong", "--expected-sha256", bundleHash]); assert.equal(JSON.parse(r.stdout).status, "confirmation_mismatch");
  ...
  for (const tag of ["v0.3.1", "0.3.1"]) { fs.rmSync(state, { force: true }); fs.rmSync(touched, { force: true }); r = invoke([], { FAKE_REF: tag }); assert.equal(JSON.parse(r.stdout).status, "update_metadata_untrusted", `bare tag ${tag} must be untrusted`); assert.doesNotMatch(fs.readFileSync(touched, "utf8"), /add/); }
