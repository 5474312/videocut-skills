...
function fakeRuntime(file, healthy = true, runtimeCapabilities = capabilities, version = "0.4.3") {
...
try {
  ...
  const releaseDirectory = path.join(tmp, "release-v0.4.3");
...
const launcher = [
  "#!/bin/sh",
  'if [ "$1" = "--version" ]; then echo "chengfeng-videocut 0.4.3"; exit 0; fi',
...
`);
  ...
  assert.match(installed.stderr, /v0\.4\.3/);
